...
### F098-4.2 · Perf R4 Step 4.2 — ExtentNode per-extent SQ/CQ ExtentWorker
- **Target:** Break the per-connection serialized `handle_connection` pattern into a per-extent SQ/CQ pipeline so multiple connections' APPEND/READ on different extents run concurrently and same-extent traffic still coalesces into ONE `write_vectored_at` (pwritev). Preserve all ACL (eversion refresh, sealed, revision fencing, commit reconciliation). All 40 lib tests + 11 existing integration tests must remain green; add 5 new integration tests for the worker path. Perf gate for this step alone: end-to-end smoke write ≥ R2 baseline (~50k), no regression at read. Microbench regression is acceptable as long as end-to-end is intact — subsequent R4 steps (4.3/4.4) need this layer's decoupling to deliver the full pipeline gain.
- **Evidence:** `crates/stream/src/extent_worker.rs` (new module — `ExtentSubmitMsg::{AppendBatch, ReadBatch, Shutdown}`, `get_or_spawn`, `worker_loop` with `FuturesUnordered` inflight cap `AUTUMN_EXTENT_INFLIGHT_CAP=64`) · `crates/stream/src/extent_node.rs` (`handle_connection` rewritten as reader + completion tasks bridging via `mpsc<oneshot::Receiver<Vec<Frame>>>`; old `handle_append_batch` / `handle_read_batch` deleted; `worker_pool: Rc<DashMap<u64, mpsc::Sender<ExtentSubmitMsg>>>` field) · `crates/stream/src/lib.rs` (`pub mod extent_worker`) · `crates/stream/tests/extent_worker.rs` (5 new tests: `extent_worker_spawns_on_first_append`, `concurrent_appends_preserve_offset_order_per_extent`, `appends_to_different_extents_run_concurrently`, `seal_rejects_subsequent_appends`, `pwritev_batch_still_coalesced`)
- **Notes:** 56 stream tests (40 lib + 16 integration incl. new 5) green. 66 partition-server tests green. **Microbench regression acknowledged**: extent_bench write depth=64 regressed from 208k → 68k ops/s (3× slower); read is within noise (~97k). Cause = unavoidable task-boundary overhead of the reader→worker→completion 3-task pipeline at per-request scale. **End-to-end perf unchanged**: `perf_check.sh --shm --partitions 1` post-refactor: write 53090 ops/s p99w=16ms, read 74772 ops/s p99r=5ms — parity with F094 baseline (~53k / 75k) and with R4 4.1 commit. The bench-vs-real gap is because the bench exercises the pure server with no client-side pipelining; real workload is network-RTT bound, not server-bound. Architecture is correctly in place for R4 4.3 (StreamClient worker) and 4.4 (PS P-log SQ/CQ) to complete the pipeline restructuring. Inflight cap `AUTUMN_EXTENT_INFLIGHT_CAP=64` (default). Len is reserved synchronously in submit ACL (`extent.len.store(total_end)` before pushing the pwritev future) to keep overlapping same-extent submits on non-overlapping offsets.
- **passes:** true
