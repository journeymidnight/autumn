...
### F087 · Stream append pipeline (方案A soft truncation + MuxPool)
- **Target:** 允许同一 log_stream 的 `append_batch` 调用并发（fast path），在保留 commit-truncation 一致性协议的前提下用 `expected_offset` 做乐观并发校验。临界区只覆盖 "分配 offset + 把 3 个 frame 推进 3 个 writer"（~30 µs），不覆盖等 ack（~1.5 ms）。服务端新增分支：reconcile=true 走 legacy 截断语义，reconcile=false 严格要求 `file_start == expected_offset`。新增 `MuxPool` 连接多路复用，`send_frame_vectored` 提交 frame 并返回 `oneshot::Receiver<Frame>`。
- **Evidence:** `crates/stream/src/extent_rpc.rs` (AppendReq 扩字段) · `crates/stream/src/extent_node.rs` (handle_append/handle_append_batch 分支) · `crates/stream/src/client.rs` (try_append_fast/append_reconcile_internal) · `crates/stream/src/conn_pool.rs` (MuxPool/MuxConn) · `autumn-rs/perf_analysis.md` (2026-04-16 方案A 落地 段) · 计划文档 `/root/.claude/plans/perf-check-running-perf-check-baseline-functional-island.md`
- **Notes:** Wire 改动：AppendReq header 29→38B，新增 `flags:u8 + expected_offset:u64` + `FLAG_RECONCILE` + `CODE_STALE_OFFSET`。客户端 `StreamAppendState` 新增 `tail_offset/reconciled/inflight`，`reconciled=true` 才能走 fast path，任一副本错 → 翻回 false 下次强制 reconcile。MAX_INFLIGHT_PER_STREAM=4。回退开关 `AUTUMN_STREAM_PIPELINE=0`。perf_check 结果：write p95 9.99→4.97ms(-50%)，p99 59→44ms(-25%)；read ops/s 56877→86960 (+53%)。write ops/s 未变——PS `background_write_loop` 的 double-buffer 设计让 inflight_peak=1，真正的 depth=2-4 吞吐提升需要后续把 write_loop 改成 ring-buffer。36 unit + 3 integration + 3 wal_recovery 测试全绿。
- **passes:** true

### F087-followup · PS background_write_loop 改为 ring-buffer
- **Target:** 把 PS `background_write_loop` 从 `Option<InFlightBatch>`（double-buffer，inflight=1）改为 `FuturesOrdered<CombinedFut>`（ring-buffer，inflight=N，默认 4）以释放 F087 stream 层 fast path 的并发能力。Phase3 仍然按 FIFO 执行（通过 FuturesOrdered 天然保证），保障 `vp_offset` 单调递增（crash-recovery 高水位）和 client ack 的顺序语义。
- **Evidence:** `crates/partition-server/src/background.rs:284-413` (ring-buffer loop + `launch_batch` 辅助函数) · `autumn-rs/perf_analysis.md` (2026-04-16 F087-followup 段) · 环境变量 `AUTUMN_PS_WRITE_INFLIGHT` (1..=16, 默认 4, =1 退回旧行为)
- **Notes:** perf_check 结果（256 threads × 30s × 4KB，localhost）：inflight_peak 1→4 达成目标，但 write ops/s 33k→35k（+5%，瓶颈转移到下游 ExtentNode 写循环）。p50 4.15→3.91ms(-6%)，p95 4.90→4.49ms(-8%)，**p99 40.70→110.59ms（+170% 恶化）** —— 深流水线放大了尾延迟（一个 Phase2 慢会阻塞整个 FuturesOrdered head 的 Phase3，4 个 batch 的 client 都等）。58 个 partition-server 单元测试全绿。A/B 验证：`AUTUMN_PS_WRITE_INFLIGHT=1` 恢复到 inflight_peak=1、p99=40.70ms 的老行为。结论：F087 方案A 的吞吐瓶颈不在 PS write loop 深度，而在 ExtentNode 端；ring-buffer 改动保留了灵活性但默认 4 会恶化 tail latency，需 F088（hedging/quorum）解决 node2 jitter 才能真正兑现 p99 降低。
- **passes:** true

### F087-bulk-mux · MuxPool 按 PoolKind 分池（Hot/Bulk）隔离 WAL 与 flush
- **Target:** 让 `log_stream`（WAL，小帧高频）与 `row_stream`/`meta_stream`（flush/checkpoint，单次 256MB）走到**不同的 TCP 连接**。之前 MuxPool 按 `SocketAddr` 索引，同一 ExtentNode 的所有 stream 共享一条 MuxConn，flush 把 `MuxConn.writer` 的 `futures::Mutex` 握住 150-200ms/replica（`write_vectored_all` 单次写 256MB），期间 log_stream 的 4KB 批都卡在 `writer.lock().await`，导致每次 flush（FLUSH_MEM_BYTES=256MB，约每 1.5s 一次）出现吞吐凹槽。新增 `PoolKind { Hot, Bulk }`，`MuxPool` 改为 `HashMap<(SocketAddr, PoolKind), Rc<MuxConn>>`；StreamClient 新增 `stream_kinds: DashMap<u64, PoolKind>` 与 `set_stream_kind()` API，默认 Hot；`fast_path` 调用处按 stream_id 查 kind。PartitionServer 在打开 partition 时登记 `row_stream_id`/`meta_stream_id` 为 Bulk。
- **Evidence:** `crates/stream/src/conn_pool.rs` (`PoolKind` 枚举 + `MuxPool::get(addr, kind)`) · `crates/stream/src/lib.rs` (re-export) · `crates/stream/src/client.rs` (`stream_kinds` 字段、`set_stream_kind`/`kind_for`、`try_append_fast` 查询) · `crates/partition-server/src/lib.rs:865-872` (`partition_thread_main` 登记 row/meta 为 Bulk)
- **Notes:** perf-check（256 threads × 10s × 4KB，3× tmpfs replicas）：write ops/s **32.5k → 42.9k (+32%)**，read 88.6k 不变；小值场景（128B）从"flush 期间整段 0 ops/s 完全停摆"变成"一个周期内从 144k 短暂跌到 20k 后立刻恢复"。write p99 依然偏高（37-81ms）——锁隔离只解决连接层 HoL，**ExtentNode 串行写盘**仍是下一个瓶颈（见 memory: `ps_write_bottleneck`）。测试全绿。代码改动零侵入：未登记的 stream 默认 Hot，向后兼容；MuxPool size 从 N=nodes 增到 2N，内存开销忽略。
- **passes:** true

### F087-mux-writer-task · MuxConn writer 去 futures::Mutex，改 mpsc + 独立 writer task
- **Target:** 在 F087-bulk-mux 的连接分池基础上，进一步拆掉 `MuxConn.writer` 的 `futures::Mutex`。旧实现中 `send_frame_vectored` 要 `writer.lock().await` 再 `write_vectored_all(bufs).await`——锁跨 await 串行化了同一 MuxConn 内多个 in-flight batch（Hot pool 典型场景：单 log_stream ring-buffer inflight=4，每个 batch 还要 fan-out 到 3 个 replica 的 3 条 MuxConn）。改为 `mpsc::UnboundedSender<Vec<Bytes>>` + spawn 一个 writer_loop task 独占 socket 写半边；`send_frame_vectored` 变成 "分配 req_id → 注册 pending → unbounded_send(bufs)"，完全不阻塞在 I/O 上。writer_loop 用 `rx.next().await` 等第一个 frame，再用 `try_recv()` 机会性 coalesce 最多 `WRITER_COALESCE_MAX=16` 帧成一次 `write_vectored_all` syscall，降低小帧场景下的 per-frame syscall 开销。
- **Evidence:** `crates/stream/src/conn_pool.rs` (`MuxConn` 结构去 `FMutex`, 新增 `writer_tx` + `writer_loop`)
- **Notes:** 语义保持：send_frame_vectored 依然返回 `oneshot::Receiver<Frame>`，失败语义从"发送时立刻 Err"转为"oneshot 上 Canceled"，上游 `client.rs` join_all 分支本来就处理了 Canceled 所以无需改动。写错误时 writer_loop 退出并清空 pending，下一次 `mux_pool.get()` 检测 `is_closed` 自动重连（复用旧路径）。coalesce 上限 16 是为了压住 iovec 数量（libc IOV_MAX）并让 reactor 间或能调度 read。perf-check 3 次（256 threads × 10s × 4KB, 3× tmpfs）：throughput 44-48k ops/s（与 F087-bulk-mux 的 44k 基线相当），**write p99 62ms → 21-22ms (-65%)**；1 次 outlier p99=115ms（tail 天然波动）。吞吐未变验证了之前的结论：44k ceiling 的瓶颈是 3× replica bytes，不是 writer mutex；但 mutex 确实是 tail latency 的 contributor——去掉后同一 MuxConn 上多个 producer 不再互相 HoL 阻塞，ack 延迟显著收敛。37 stream + 58 PS 单元测试全绿。
- **passes:** true
