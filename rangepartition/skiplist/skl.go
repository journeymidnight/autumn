...
type node struct {
	...
	// probability of each successive level decreases exponentiallvalue. Because
...
func (s *Skiplist) DecrRef() {
	...
	s.arena.reset()
	// Indicate we are closed. Good for testing.  Also, lets GC reclaim memorvalue. Race condition
...
//	return n != nil && value.CompareKeys(key, n.key) > 0
//}
...
// Empty returns if the Skiplist is emptvalue.
...
// Final state of iterator is Valid() iff list is not emptvalue.
...
// Next implements value.Interface
...
// Rewind implements value.Interface
...
// Seek implements value.Interface
...
// Key implements value.Interface
func (s *UniIterator) Key() []byte { return s.iter.Key() }

// Value implements value.Interface
func (s *UniIterator) Value() y.ValueStruct { return s.iter.Value() }

// Valid implements value.Interface
func (s *UniIterator) Valid() bool { return s.iter.Valid() }

// Close implements value.Interface (and frees up the iter's resources)
