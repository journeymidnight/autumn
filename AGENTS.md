...
1. 每次开始任务前，必须先读取仓库根目录 `progress.md` 与 `feature_list.md`。
2. 在真正编码前，先输出本任务的两份清单：
   - 已实现的 feature/task
   - 未实现的 feature/task
3. 清单输出后才能开始编码。
4. 编码完成后必须更新 `progress.md` 中的任务状态，状态值只允许两种：
   - `completed`
   - `not_completed`
