...
11. **PoolKind stream affinity (Hot vs Bulk)** — `ConnPool` keys by `(SocketAddr, PoolKind)` with `PoolKind ∈ {Hot, Bulk}`. Each (addr, kind) pair owns a distinct sequential `RpcConn` (take/put pattern on `Rc<RefCell<Option<RpcConn>>>`; if taken, a fresh connection is opened on the fly). Callers register per-stream affinity via `StreamClient::set_stream_kind(stream_id, PoolKind::Bulk)`; unregistered streams default to `Hot`. The PartitionServer assigns `log_stream` = Hot (small frequent WAL frames) and `row_stream`/`meta_stream` = Bulk (128MB+ SSTable / checkpoint uploads). Without this split, a single 128MB `write_vectored_all` on the shared socket serializes behind every small WAL batch, causing sawtoothed throughput synchronized to flush cycles (~+32% throughput for the 4KB mixed workload).
