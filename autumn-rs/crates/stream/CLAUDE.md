...
11. **PoolKind stream affinity (Hot vs Bulk)** — `MuxPool` keys by `(SocketAddr, PoolKind)`. Each (addr, kind) pair owns a distinct TCP connection with its own writer task. Callers register per-stream affinity via `StreamClient::set_stream_kind(stream_id, PoolKind::Bulk)`; unregistered streams default to `Hot`. The PartitionServer assigns `log_stream` = Hot (small frequent WAL frames) and `row_stream`/`meta_stream` = Bulk (256MB SSTable / checkpoint uploads). Without this split, a single large `write_vectored_all` on the shared MuxConn blocks the writer task for hundreds of ms and head-of-line-blocks every small WAL batch, causing sawtoothed throughput synchronized to flush cycles.

12. **MuxConn writer is task-owned, not mutex-protected** — Inside each `MuxConn`, the socket write half is owned by a spawned `writer_loop` task that drains an `mpsc::UnboundedReceiver<Vec<Bytes>>`. `send_frame_vectored` is effectively synchronous: allocate `req_id`, insert into `pending`, `unbounded_send(bufs)`. No writer lock is held across the I/O await, so multiple in-flight producers on the same MuxConn never HoL-block each other. The writer task opportunistically coalesces up to `WRITER_COALESCE_MAX=16` queued frames per `write_vectored_all` syscall to amortize syscall cost under bursty small-frame load. On write error, the task sets `closed=true` and clears `pending`; pending oneshots resolve to `Canceled`, callers handle that branch the same as before. This removes the `futures::Mutex` that previously contributed to write p99 spikes on Hot pool under ring-buffer inflight=4.
